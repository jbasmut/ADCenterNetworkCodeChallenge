...
{
    public class Sequence_mRNAStreamService
    {
        private readonly ILogger<Sequence_mRNAStreamService> _logger;

        public Sequence_mRNAStreamService(ILogger<Sequence_mRNAStreamService> logger)
        {
            _logger = logger;
        }

        public Task<string> CheckSequence_mRNA(string path, int counter)
        {
            // Get the txt file from path
            // Read the file as one string.
            string sequence = System.IO.File.ReadAllText(@path);            
            ...
            string gene = genes[counter];

            return Task.FromResult(gene);
        }
